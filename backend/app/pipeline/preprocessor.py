...
# Extended column aliases covering major Kaggle retail/inventory datasets
COLUMN_ALIASES = {
    "date": [
...
class DataPreprocessor:
    """
    Cleans and prepares inventory data from any CSV or JSON source.
    Handles Kaggle datasets, custom company data, and any column naming convention.
    Auto-generates missing store_id or product_id if not found.
    """
    ...
    def get_column_mapping(self, df: pd.DataFrame) -> dict:
        """Return what columns were detected and mapped — for frontend preview."""
        rename_map = self._build_rename_map(df)
        detected = {v: k for k, v in rename_map.items()}
        missing = []
        for col in REQUIRED_COLS:
            if col not in rename_map.values() and col not in df.columns:
                if col == "store_id":
                    missing.append({"column": col, "action": "auto-generated as 'DEFAULT_STORE'"})
                elif col == "product_id":
                    missing.append({"column": col, "action": "auto-generated from row index"})
                else:
    ...
    def _normalize_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Fuzzy fallback — try lowercase strip match
    ...
    def _auto_generate_missing(self, df: pd.DataFrame) -> pd.DataFrame:
        """Auto-generate store_id and product_id if not found in data."""
        if "store_id" not in df.columns:
            df["store_id"] = "STORE_001"
            logger.info("store_id not found — auto-generated as STORE_001")

        if "product_id" not in df.columns:
            # Try to use category or description as product_id
            for candidate in ["category", "description", "name", "item", "sku"]:
                if candidate in df.columns:
                    df["product_id"] = df[candidate].astype(str).str.upper().str.replace(" ", "_")
        ...
        if "units_sold" not in df.columns:
            # Try to find any numeric column as sales proxy
